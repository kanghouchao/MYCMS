...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        if (tenantContext.isTenant()) {
            ...
            return centralUserRepository.findByUsername(username)
                    ...
                    .orElseThrow(() -> new UsernameNotFoundException(
                            "User not found (central): " + username));
        }
    }
    ...
    private <R, P> List<SimpleGrantedAuthority> buildAuthorities(
        ...
        return roles.stream()
                .flatMap(r -> {
                    ...
                    Stream<String> permOriginal = perms == null ? Stream.empty()
                            : perms.stream().map(permNameExtractor);
                    Stream<String> permWithPrefix = perms == null ? Stream.empty()
                            : perms.stream().map(p -> "PERM_" + (permNameExtractor.apply(p) == null ? ""
    ...
    private UserDetails buildUser(String username, String password, boolean enabled,
