...
class CentralAuthServiceImplTest {
    ...
    @Captor
    ArgumentCaptor<String> issuerCaptor;
    @Captor
    ArgumentCaptor<Map<String, Object>> claimsCaptor;

    @InjectMocks
    CentralAuthServiceImpl service;
    ...
    void loginGeneratesTokenWithAuthorities() {
        ...
        when(authentication.getAuthorities()).thenAnswer(inv -> List.<GrantedAuthority>of(() -> "ROLE_ADMIN"));
        ...
        verify(jwtUtil).generateToken(subjectCaptor.capture(), issuerCaptor.capture(), claimsCaptor.capture());
