...
export async function middleware(request: NextRequest) {
    ...
    const validationApiUrl = process.env.TENANT_VALIDATION_API_URL || "http://backend:8000/api/tenant";
    ...
    if (!res.ok || !data || !data.tenant_id) {
        return NextResponse.redirect(new URL((process.env.APP_URL || "//oli-cms.test") + "/welcome", request.url));
    }
