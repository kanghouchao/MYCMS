...
implementation and agent-run patterns should reference the copilot
instructions to avoid duplication.

## Pull Request Template (canonical reference)

The repository's canonical PR template is maintained in the file
`.github/pull_request_template.md`. To ensure consistent reviews and that Quality
Gates are provided, PR authors MUST read and use that template when creating a PR.

If a PR cannot reasonably include the template (for example: an automated hotfix
where the PR body is generated by CI), the author MUST add a short justification in
the PR body explaining why the template was not used.

Maintainers may request edits or close PRs that omit required Quality Gates or lack
adequate smoke validation steps.

**Version**: 1.1.0 | **Ratified**: TODO(RATIFICATION_DATE) | **Last Amended**: 2025-09-20
